...
const SeverityIcon = ({ severity }: { severity: string }) => {
  ...
  if (severity === "Warning") return <AlertTriangle style={{ color: "var(--warning)", width: 16, height: 16 }} />;
  return <CheckCircle style={{ color: "var(--success)", width: 16, height: 16 }} />;
};
...
const AlertRow = ({ alert, onAcknowledge }: { alert: Alert; onAcknowledge: (alert: Alert) => void }) => (
  <TooltipProvider delayDuration={200}>
    <Tooltip>
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className={rowClass(alert)}>
          <div className="alert-row-inner">
            ...
            <div className="alert-row-actions">
              {!alert.acknowledged && (
                <Button size="sm" variant="outline" className="h-7 text-xs" onClick={(e) => { e.stopPropagation(); onAcknowledge(alert); }}>
                  Acknowledge
                </Button>
              )}
      ...
      <TooltipContent side="top" className="alert-ack-tooltip" style={{ maxWidth: "20rem", fontSize: "0.75rem", padding: "0.75rem" }}>
        ...
        {alert.currentValue != null && (
          <>
            ...
            <p>Deviation: <span className="mono" style={{ color: "var(--critical)" }}>
