                    ...
                    {editingIndex === i ? (
                      ...
                      <>
                        ...
                        <div className="settings-sku-actions">
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => startEdit(i)}><Pencil /></Button>
                          <Button size="icon" variant="ghost" className="h-7 w-7" style={{ color: "var(--destructive)" }} onClick={() => deleteSku(i)}><Trash2 /></Button>
                        </div>
                      </>
                    )}
                  </div>
          ...
          <TabsContent value="tariff">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="chart-container settings-section">
              ...
              <div className="settings-grid-3">
                ...
                <div className="settings-field">
                  <Label className="text-xs">Fixed Charges (₹)</Label>
                ...
                <div className="settings-field">
                  ...
                  <Popover open={tariffStartOpen} onOpenChange={setTariffStartOpen}>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="h-9 text-sm" style={{ width: "100%", justifyContent: "flex-start", gap: 8 }}>
                        <CalendarIcon style={{ width: 16, height: 16, color: "var(--muted-foreground)" }} />
                        {tariffStart ? format(tariffStart, "dd MMM yyyy") : "Pick date"}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar mode="single" selected={tariffStart} onSelect={(d) => { if (d) { setTariffStart(d); setTariffStartOpen(false); } }} initialFocus className="p-3 pointer-events-auto" />
                    </PopoverContent>
                  </Popover>
                </div>
                <div className="settings-field">
                  <Label className="text-xs">Effective End Date <span style={{ color: "var(--muted-foreground)" }}>(optional)</span></Label>
                  <Popover open={tariffEndOpen} onOpenChange={setTariffEndOpen}>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar mode="single" selected={tariffEnd} onSelect={(d) => { setTariffEnd(d); setTariffEndOpen(false); }} initialFocus className="p-3 pointer-events-auto" />
                    </PopoverContent>
                  </Popover>
                </div>
                <div className="settings-field-flex-end">
                  <Button onClick={addTariff} className="h-9 gap-1"><Plus />Add Tariff</Button>
                </div>
              </div>
                      ...
                      <div key={slot.label} className="settings-tod-slot">
                        <div className="settings-tod-slot-field">
                          ...
                          <div className="settings-tod-slot-pill">
                            <Badge variant="outline" className={slot.label === "Peak" ? "settings-badge-peak" : "settings-badge-offpeak"} style={{ fontSize: 11 }}>
                              {slot.label} Hours
                            </Badge>
                          </div>
                        </div>
              ...
              <div className="settings-section" style={{ gap: "0.5rem" }}>
                    ...
                    <tbody>
                      {tariffs.map((t) => (
                        <tr key={t.id}>
                          ...
                          <td><Badge variant="outline" style={{ fontSize: 10 }}>{t.type}</Badge></td>
                          ...
                          <td className="right">
                            <Button size="icon" variant="ghost" className="h-7 w-7" style={{ color: "var(--destructive)" }} onClick={() => deleteTariff(t.id)}><Trash2 /></Button>
                          </td>
                        </tr>
                      ))}
                      {tariffs.length === 0 && (
          ...
          <TabsContent value="moisture-specs">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="chart-container settings-section">
              ...
              <div className="settings-grid-3-cells">
                <div className="settings-field"><Label className="text-xs">LSL (Lower Spec Limit, %)</Label><Input type="number" step="0.1" value={specLsl} onChange={(e) => setSpecLsl(e.target.value)} /></div>
                <div className="settings-field"><Label className="text-xs">USL (Upper Spec Limit, %)</Label><Input type="number" step="0.1" value={specUsl} onChange={(e) => setSpecUsl(e.target.value)} /></div>
                <div className="settings-field"><Label className="text-xs">Target (%)</Label><Input type="number" step="0.1" value={specTarget} onChange={(e) => setSpecTarget(e.target.value)} /></div>
              </div>
          ...
          <TabsContent value="process">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="chart-container settings-section">
              ...
              <div className="settings-process-block">
                ...
                <div className="settings-grid-3-cells">
                  <div className="settings-field"><Label className="text-xs">Target (°C)</Label><Input defaultValue="85" type="number" /></div>
                  <div className="settings-field"><Label className="text-xs">LSL (°C)</Label><Input defaultValue="75" type="number" /></div>
                  <div className="settings-field"><Label className="text-xs">USL (°C)</Label><Input defaultValue="95" type="number" /></div>
                </div>
              </div>
              <Separator />
              <div className="settings-process-block">
                ...
                <div className="settings-grid-3-cells">
                  <div className="settings-field"><Label className="text-xs">Target (% RH)</Label><Input defaultValue="58" type="number" /></div>
                  <div className="settings-field"><Label className="text-xs">LSL (% RH)</Label><Input defaultValue="50" type="number" /></div>
                  <div className="settings-field"><Label className="text-xs">USL (% RH)</Label><Input defaultValue="65" type="number" /></div>
                </div>
              </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="chart-container settings-section">
              ...
              <div className="settings-upload-zone">
                ...
                <Button variant="outline" onClick={() => fileInputRef.current?.click()}>
                  <Upload style={{ marginRight: 6 }} />Select File
                </Button>
              </div>
            </motion.div>
          </TabsContent>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="chart-container settings-section">
              ...
              <div className="settings-grid-2">
                ...
                <div className="settings-ec-card dashed">
                  ...
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => document.getElementById("ec-losses-file")?.click()}>
                    <Upload /> Select File
                  </Button>
                </div>
              </div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="chart-container settings-section">
          ...
          <tbody>
            ...
            {rules.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  <Badge variant="outline" className={r.severity === "Critical" ? "settings-badge-peak" : "settings-badge-offpeak"} style={{ fontSize: 11 }}>{r.severity}</Badge>
                </td>
                ...
                <td>
                  <div style={{ display: "flex", gap: "0.25rem", justifyContent: "flex-end" }}>
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => openEdit(r)}><Pencil /></Button>
                    <Button size="icon" variant="ghost" className="h-7 w-7" style={{ color: "var(--destructive)" }} onClick={() => removeRule(r.id)}><Trash2 /></Button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[640px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="settings-section" style={{ gap: "1rem" }}>
            <div className="settings-field">
            ...
            <div className="settings-field" style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: "0.5rem" }}>
              <Checkbox id="useLimits" checked={draft.useLimits} onCheckedChange={(v) => setDraft({ ...draft, useLimits: v === true })} />
              <Label htmlFor="useLimits" className="text-xs" style={{ cursor: "pointer" }}>LCL/UCL</Label>
            </div>
            ...
            <div className="settings-field">
              <Label className="text-xs" style={{ display: "flex", alignItems: "center", gap: "0.375rem" }}><Mail style={{ width: 14, height: 14 }} /> Email Notification Recipients</Label>
              <div className="settings-add-row">
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", marginTop: "0.25rem" }}>
                ...
                {draft.emails.map((e) => (
                  <Badge key={e} variant="outline" style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", padding: "0.125rem 0.5rem" }}>
                    {e}
                    <button onClick={() => removeEmail(e)} style={{ display: "inline-flex", background: "transparent", border: 0, cursor: "pointer", color: "var(--muted-foreground)" }}>
            ...
            <div className="settings-field">
              <Label className="text-xs">Alert Interval (HH:MM)</Label>
            ...
            <div className="settings-field">
              <Label className="text-xs">Scheduled Summary Notifications</Label>

              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", border: "1px solid var(--border)", borderRadius: 8, padding: "0.75rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <Checkbox id="daily-check" checked={draft.daily.enabled} onCheckedChange={(v) => setDraft({ ...draft, daily: { ...draft.daily, enabled: !!v } })} />
                  <Label htmlFor="daily-check" className="text-sm" style={{ cursor: "pointer" }}>Daily summary</Label>
                </div>
                {draft.daily.enabled && (
                  <div style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: "0.75rem", paddingLeft: "1.5rem" }}>
                    <div className="settings-field">
                    ...
                    <div className="settings-field">
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", marginTop: "0.25rem" }}>
                        ...
                        {draft.daily.users.map((u) => (
                          <Badge key={u} variant="outline" style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", padding: "0.125rem 0.5rem" }}>
                            {u}
                            <button onClick={() => removeDailyUser(u)} style={{ display: "inline-flex", background: "transparent", border: 0, cursor: "pointer", color: "var(--muted-foreground)" }}>
                              <X style={{ width: 12, height: 12 }} />
                            </button>
                          </Badge>
                        ))}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  ...
                  <Label htmlFor="shift-check" className="text-sm" style={{ cursor: "pointer" }}>Shift wise summary</Label>
                </div>
                {draft.shiftWise.enabled && (
                  <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: "0.75rem", paddingLeft: "1.5rem" }}>
                    <div className="settings-field">
                    ...
                    <div className="settings-field">
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", marginTop: "0.25rem" }}>
                        ...
                        {draft.shiftWise.users.map((u) => (
                          <Badge key={u} variant="outline" style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", padding: "0.125rem 0.5rem" }}>
                            {u}
                            <button onClick={() => removeShiftUser(u)} style={{ display: "inline-flex", background: "transparent", border: 0, cursor: "pointer", color: "var(--muted-foreground)" }}>
                              <X style={{ width: 12, height: 12 }} />
                            </button>
                          </Badge>
                        ))}
            ...
            <div className="settings-notif-row">
              <Label className="text-sm">Enable this alert</Label>
